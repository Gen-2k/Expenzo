...
import { User, Bell, Shield, Moon, Globe, LogOut, ChevronRight, CreditCard, HelpCircle } from 'lucide-react';
    ...
    const sections = [
        {
            title: 'Account',
            items: [
                { icon: User, label: 'Personal Information', desc: 'Name, Email, Profile Photo' },
        ...
        },
        {
            title: 'Preferences',
            items: [
                { icon: Bell, label: 'Notifications', desc: 'Customize your alerts', toggle: true },
                { icon: Moon, label: 'Dark Mode', desc: 'Switch to dark theme', toggle: true },
         ...
         <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="text-center md:text-left flex-1">
                ...
                <div className="mt-4 flex gap-3 justify-center md:justify-start">
                    <button className="px-5 py-2 rounded-xl bg-gray-900 text-white text-sm font-semibold hover:bg-gray-800 transition-colors">Edit Profile</button>
                    <button className="px-5 py-2 rounded-xl border border-gray-200 text-gray-700 text-sm font-semibold hover:bg-gray-50 transition-colors">Change Plan</button>
                </div>
            </div>
         </div>
                ...
                <div key={section.title} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <div className="divide-y divide-gray-50">
                        {section.items.map((item) => (
                            <div key={item.label} className="p-4 md:p-5 flex items-center justify-between hover:bg-gray-50 transition-colors cursor-pointer group">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500 hidden md:block">{item.desc}</p>
                                    </div>
                                </div>
                                
                                {item.toggle ? (
                                    <div className="relative w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full">

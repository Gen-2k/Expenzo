...
import { Plus, AlertCircle, CheckCircle, TrendingUp, ShoppingBag, Coffee, Car, Home, Zap } from 'lucide-react';

const Budgets = () => {
  // Mock Budget Data
  const budgets = [
    { id: 1, category: 'Food & Dining', spent: 12450, limit: 15000, icon: Coffee, color: 'bg-orange-100 text-orange-600', hue: 'orange' },
    { id: 2, category: 'Transportation', spent: 4200, limit: 5000, icon: Car, color: 'bg-blue-100 text-blue-600', hue: 'blue' },
    { id: 3, category: 'Shopping', spent: 8500, limit: 8000, icon: ShoppingBag, color: 'bg-purple-100 text-purple-600', hue: 'purple' },
    { id: 4, category: 'Bills & Utilities', spent: 3200, limit: 6000, icon: Zap, color: 'bg-yellow-100 text-yellow-600', hue: 'yellow' },
    { id: 5, category: 'Rent', spent: 15000, limit: 15000, icon: Home, color: 'bg-indigo-100 text-indigo-600', hue: 'indigo' },
  ];
    ...
    <Layout>
      <div className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
            <div>
                <h1 className="mb-2">Monthly Budgets</h1>
                <p>Set limits to stay in control of your spending.</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-3xl p-6 text-white shadow-xl shadow-indigo-600/20 relative overflow-hidden">
                ...
                <p className="text-indigo-100 font-medium text-sm">Total Budgeted</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {budgets.map((budget) => {
                ...
                return (
                    <div key={budget.id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm transition-all group">
                        <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${budget.color} transition-colors`}>
                                    <budget.icon className="w-6 h-6" />
                                </div>
                                <div>
                                    <h3 className="font-bold text-gray-900">{budget.category}</h3>
                                    <p className="text-xs text-gray-500 mt-1">₹{budget.spent.toLocaleString()} of ₹{budget.limit.toLocaleString()}</p>
                                </div>
                            </div>
                            {isOver ? (
                        ...
                        <div className="relative pt-2">
                            <div className="flex justify-between text-xs font-semibold mb-2">
                                <span className={isOver ? 'text-red-500' : 'text-gray-500'}>{Math.round(percent)}%</span>
                                <span className="text-gray-400">Target: ₹{budget.limit.toLocaleString()}</span>
                            </div>
                            <div className="h-3 w-full bg-gray-100 rounded-full overflow-hidden">
                                <div 
                                    className={`h-full rounded-full transition-all duration-1000 ${
                                        isOver ? 'bg-red-500' : 
                                        percent > 85 ? 'bg-yellow-500' : 
                                        'bg-primary-600'
                                    }`} 
                                    style={{ width: `${percent}%` }}
                                />
